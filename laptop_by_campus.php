...
// Fetch laptops for the specified location
$sql = "SELECT id, name, laptop_model, serial_number, location, laptop_status 
        FROM users 
        WHERE location = ? AND laptop_model != ''";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $location);
$stmt->execute();
$laptops = $stmt->get_result();

// Get the friendly name for the location
$location_names = [
    'SMI' => 'Munawwar Campus',
    'RSK' => 'Korangi Campus',
...
$display_name = $location_names[$location] ?? 'Unknown Location';
...
<head>
    ...
    <title><?php echo htmlspecialchars($display_name); ?> - Laptops</title>
    <style>
        body {
...
<!-- Navigation Bar -->
    ...
    <div class="container">
        <h1>Laptops Assigned to <?php echo htmlspecialchars($display_name); ?></h1>
        <table class="table">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr onclick="showDetails(<?php echo $laptop['id']; ?>)">
                    <td class="hidden"><?php echo htmlspecialchars($laptop['id']); ?></td>
    ...
    <script>
        function showDetails(userId) {
            window.location.href = 'check_payment_type.php?user_id=' + userId;
        }
    </script>
</body>
</html>